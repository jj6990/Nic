...
$(function(){
    if($.browser.msie && $.browser.version <= 9){
        ...
        $("input[placeholder]").parents("form").find(button[type='submit']).click(function() {
            $(this).siblings('input[placeholder]').each(function() {
...
$(document).ready(function(){
    ...
    if(typeof highlight_preferred_bundle_plan == 'undefined'){
        $("div.new-homepage-wrapper, .new-homepage-wrapper-dark-banner").removeClass("add-top-space");
    }
    else if(highlight_preferred_bundle_plan == '' || highlight_preferred_bundle_plan == 'false'){
        $("div.new-homepage-wrapper, .new-homepage-wrapper-dark-banner").removeClass("add-top-space");
    } else {
        $("div.new-homepage-wrapper, .new-homepage-wrapper-dark-banner").addClass("add-top-space");
    }
    ...
    if($("input#field_domains-input").length) {
        ...
        $('#field_domains-input').textboxValueToggle($("#default_domain_search").val());
...
function check_domain_name() // error msg for domian defult value
{
    ...
    if((domain_name.length == 0) || (domain_name.match(/[^a-zA-Z0-9-.\s]+/)) || domain_name == $("#default_domain_search").val())
    {
